...
import time
from ChineseTextClassificationPytorch.utils import get_time_dif
...
def predict(model, data_iter):
    ...
    with open(TEST_UNIFIED_FILE, encoding='utf-8') as test_file:
        with open(RESULT_DATA_FILE, 'w', encoding='utf-8') as result_file:
            result_file.write('id,label\n')
            df = csv.reader(test_file, delimiter='\t')
            index = 0
            for i in islice(df, 1, None):
                # print(predict_all[index])
                result_file.write(i[0] + ',' + str(predict_all[index]) + '\n')
                index += 1
