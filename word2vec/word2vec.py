...
def get_fact():
    ...
    result = []
    print('begin reading train file')
    with open(TRAIN_DATA_FILE, "r", encoding="UTF-8") as train_data:
        content = [row[CONTENT_LOC] for row in csv.reader(train_data)][1:]
        for text in content:
...
def regular_filter(word):
    if re.match(r'.*某', word) is not None or re.match(r'某.*', word) is not None or re.match(r'.*县',
                                                                                            word) is not None or re.match(
        r'.*乡', word) is not None:
        return False
    return True
